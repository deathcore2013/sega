...
namespace ImGui
{
	...
	//IMGUI_API bool          SliderFloatOT(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", float power = 1.0f);
	...
	// Widgets: Main
	IMGUI_API bool OwOButton(const char* label, bool inuse, const ImVec2& size_arg);
	...
	IMGUI_API bool BeginChildOT(const char* str_id, const ImVec2& size_arg, bool border = false, ImGuiWindowFlags flags = 0);
	IMGUI_API bool          CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value);
	IMGUI_API bool          RadioButton(const char* label, bool active);
	IMGUI_API bool          Checkbox(const char* label, bool* v, int max);
	...
	//IMGUI_API bool          SliderInt(const char* label, int* v, int v_min, int v_max, const char* display_format = "%.0f", float width = 200);
	IMGUI_API bool          MyKnob(const char* label, float* p_value, float v_min, float v_max);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_ChildWindowAutoFitY = 1 << 30,
	 ImGuiWindowFlags_GroupBox = 1 << 29   // Don't use! For internal use by BeginGroupBox()
