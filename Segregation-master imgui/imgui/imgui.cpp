...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
	...
	{
		ImGui::GetWindowDrawList()->AddRectFilledMultiColor(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1),  ImColor(0, 255, 0, 55), ImColor(0, 255, 0, 55),  rounding, ImDrawCornerFlags_All);
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	//ImGui::GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(2, 18), ImColor(0, 0, 0, 255), ImColor(255, 255, 255, 255), ImColor(255, 0, 255, 255), ImColor(255, 0, 255, 255));
	//parent_window->DrawList->AddRectFilledMultiColor(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + 16), ImColor(0, 0, 0, 55), ImColor(0, 0, 0, 55), ImColor(0, 0, 0, 55), ImColor(255, 255, 255, 55));
	parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x, child_bb.y + size_arg.y), ImColor(0.8f, 0.8f, 0.8f, 0.3f));
	parent_window->DrawList->AddLine(ImVec2(child_bb.x + size_arg.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(0.8f, 0.8f, 0.8f, 0.3f));
	parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y + size_arg.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(0.8f, 0.8f, 0.8f, 0.3f));
	parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + 10, child_bb.y), ImColor(0.8f, 0.8f, 0.8f, 0.3f));
	parent_window->DrawList->AddLine(ImVec2(child_bb.x + 20 + value_bufsize.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y), ImColor(0.8f, 0.8f, 0.8f, 0.3f));
	parent_window->DrawList->AddText(ImVec2(child_bb.x + 15, child_bb.y - 6), ImColor(0.8f, 0.8f, 0.8f, 0.7f), name);
...
bool ImGui::beginchildex2(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
	...
	flags |= ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_ChildWindow;
...
{
	ImGuiWindow* window = GetCurrentWindow();
	return beginchildex2(str_id, window->GetID(str_id), size_arg, sub_tab, extra_flags);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (held)
	{
		//window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImVec4(55,252,0,255)), 0);
	}
	else
	{
		window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(ImVec4(0,0,0,2)), GetColorU32(ImVec4(0,0,0,2)), GetColorU32(ImVec4(251,0,255,2)), GetColorU32(ImVec4(251, 0, 255, 2)));
	}
	...
	window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(255,255,255,255)), 0);
	...
	float radius = max(fabs(it_circle->second.pos.x - bb.Max.x), fabs(it_circle->second.pos.x - bb.Min.x)) * 2.f;
	...
	ImVec4 hovering_color = ImVec4(255, 255, 255, it_hover->second * 0.4f);
	ImVec4 hovering_color1 = ImVec4(255, 255, 255, it_hover->second * 0.4f);
	ImVec4 hovering_color2 = ImVec4(0, 0, 0, it_hover->second * 0.4f);
	window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(hovering_color), GetColorU32(hovering_color1), GetColorU32(hovering_color2), GetStyle().FrameRounding);


	//window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(hovering_color), GetStyle().FrameRounding);
	window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Max, 255, 255, 255, 255);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb_slider.Min, bb_slider.Max,  ImColor(0, 0, 0, 55), ImColor(0, 0, 0, 255), ImColor(0, 0, 0, 255), ImColor(0, 0, 0, 255));
	PopClipRect();
	//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(-1, 4), frame_bb.Max - ImVec2(1, 17), ImColor(0, 0, 0, 255));

			//ImGui::GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 5), frame_bb.Max - ImVec2(2, 18), ImColor(0, 0, 0, 255), ImColor(255, 255, 255, 255), ImColor(255, 0, 255, 255), ImColor(255, 0, 255, 255));

	//ImGui::GetWindowDrawList()->AddRectFilledMultiColor(grab_bb.Min - ImVec2(2, 3), grab_bb.Max + ImVec2(2, 3), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 255));
	ImGui::GetWindowDrawList()->AddRectFilledMultiColor(grab_bb.Min - ImVec2(1, 2), grab_bb.Max + ImVec2(1, 2), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), ImColor(0, 0, 0, 255),  ImColor(0, 0, 0, 255), ImColor(0, 0, 0, 255));
...
//	bool hovered, held;
	...
	//RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 5.f));
	...
	window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(255, 255, 255, 255)), 0);
	if (hovered && !it_circle->second.clicked)
	{
		it_circle->second.clicked = true;
		it_circle->second.pos = GetIO().MousePos;
	}
	...
	// If pressed and was active - restart animation
	else if (hovered && it_circle->second.clicked)
	{
		it_circle->second.mult = 0.f;
		it_circle->second.size = 0.f;
	...
	// Circle radius calcalution
	float radius = max(fabs(it_circle->second.pos.x - frame_bb.Max.x), fabs(it_circle->second.pos.x - frame_bb.Min.x)) * 2.f;

	// Clip area to draw a cicrle inside frame
	//window->DrawList->PushClipRect(frame_bb.Min, frame_bb.Max);
	//->DrawList->AddCircleFilled(it_circle->second.pos, radius * it_circle->second.size, GetColorU32(circle_color), 30);
	//window->DrawList->PopClipRect();
	...
	ImVec4 hovering_color1 = ImVec4(255, 255, 255, it_hover->second * 0.4f);
	ImVec4 hovering_color2 = ImVec4(0, 0, 0, it_hover->second * 0.4f);
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, GetColorU32(hovering_color), GetColorU32(hovering_color1), GetColorU32(hovering_color2), GetStyle().FrameRounding);

	//char value_buf[64];
	//const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), display_format, *v);
	//RenderText(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x, frame_bb.Min.y - 13), value_buf, value_buf_end);
	// RenderTextClipped( frame_bb.Min - ImVec2( 0, 17 ), frame_bb.Max - ImVec2( 0, 14 ), value_buf, value_buf_end, NULL, ImVec2( 1.f, 1.f ) );

	//if (label_size.x > 0.0f)
		//RenderText(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y - 13), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const float check_sz2 = check_sz / 1;
	const float pad = ImMax(0.0f, (float)(int)(check_sz / 2.5f));
	if (*v)
	{

		RenderFrame(check_bb.Min, check_bb.Max, ImGui::GetColorU32(ImVec4(255,255,255,255)),  true, style.FrameRounding);

		const float pad = ImMax(1.0f,ImFloor(square_sz / 2.f));
	...
	if (*v)
	{
		...
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + 4 + i, check_bb.Min.y + (check_bb.GetHeight() / 2)), ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Min.y + 4), color_grey);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + 4 + i, check_bb.Min.y + (check_bb.GetHeight() / 2)), ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Max.y - 4), color_grey);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Min.y + 4), ImVec2(check_bb.Max.x - 4 - i, check_bb.Min.y + (check_bb.GetHeight() / 2)), color_grey);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Max.y - 4), ImVec2(check_bb.Max.x - 4 - i, check_bb.Min.y + (check_bb.GetHeight() / 2)), color_grey);
		//on
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), GetColorU32(ImVec4(0, 255, 0, 255)), style.FrameRounding);
	...
	{
		auto color_black_off = ImGui::GetColorU32(ImVec4(255, 255, 255, 255));
		auto color_grey_off = ImGui::GetColorU32(ImVec4(255, 0, 255, 255));
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x, check_bb.Min.y + (check_bb.GetHeight() / 2)), ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Min.y), GetColorU32(ImVec4{ 1,1,1,1 }), 2);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x, check_bb.Min.y + (check_bb.GetHeight() / 2)), ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Max.y), GetColorU32(ImVec4{ 1,1,1,1 }), 2);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Min.y), ImVec2(check_bb.Max.x, check_bb.Min.y + (check_bb.GetHeight() / 2)), GetColorU32(ImVec4{ 1,1,1,1 }), 2);
		//window->DrawList->AddLine(ImVec2(check_bb.Min.x + (check_bb.GetWidth() / 2), check_bb.Max.y), ImVec2(check_bb.Max.x, check_bb.Min.y + (check_bb.GetHeight() / 2)), GetColorU32(ImVec4{ 1,1,1,1 }), 2);
		//off
...
void RenderCheckMark2(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz)
{
	float thickness = ImMax(sz / 5.0f, 1.0f);
	sz -= thickness * 0.5f;
	pos += ImVec2(thickness * 0.25f, thickness * 0.25f);
...
bool ImGui::Checkbox2(const char* label, bool* v)
{
	...
	//ImGui::PushFont(InterMedium);
	{
		...
		if (*v)
			RenderCheckMark2(window->DrawList, { total_bb.Max.x - square_sz / 1.4f, total_bb.Min.y + square_sz / 3.1f }, ImColor(155, 212, 90), 10.f);
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags, int mouse_button)
{
	...
	if (source_drag_active)
	{
		...
		if (!(flags & ImGuiDragDropFlags_SourceNoPreviewTooltip))
		{
			...
			PushStyleColor(ImGuiCol_PopupBg, GetStyleColorVec4(ImGuiCol_PopupBg) * ImVec4(0,0,0,255));
			BeginTooltip();
		}
